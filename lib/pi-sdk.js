...
export const PI_AUTH_SCOPES = ["username"]
...
/** T2P Proof is on Pi Testnet — sandbox must be true */
...
async function initPiWithTimeout(pi, config) {
  await Promise.race([
    Promise.resolve(pi.init(config)),
...
export async function ensurePiInitialized() {
  ...
  if (!initPromise) {
    initPromise = initPiWithTimeout(pi, {
      version: "2.0",
...
export function startPlainOAuthSignIn(options = {}) {
  if (!isPiOAuthConfigured()) {
    throw new Error("Pi OAuth client ID is not configured.")
  }

  const returnTo = window.location.href
  const state = encodeOAuthState(returnTo, options)
...
export async function piAuthenticate(scopes, onIncompletePaymentFound = () => {}) {
...
/** Pi Browser → authenticate(). Safari/Chrome → OAuth redirect only. */
export async function piSignIn(options = {}) {
  if (isNativePiEnv() || hasPiSdk()) {
    ...
    const auth = await piAuthenticate(PI_AUTH_SCOPES)
    return { method: "authenticate", auth }
  }

  startPlainOAuthSignIn(options)
